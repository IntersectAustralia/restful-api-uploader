...
require 'rubygems'
require File.expand_path(File.dirname(__FILE__) + '/../lib/wrapper_uploader')

describe BatchUploader do
  let(:wrapper_template_config_path) { File.expand_path(File.dirname(__FILE__) + '/resources/wrapper_config.yml') }
  let(:wrapper_spec_config_path) { File.expand_path(File.dirname(__FILE__) + '/../tmp/wrapper_config.yml') }
  ...
  before do
    ...
    rewrite_paths(wrapper_template_config_path, wrapper_spec_config_path, [sample1_path, sample2_path])
  ...
  describe 'Reading the config' do
    it 'should read the YAML wrapper configuration correctly' do
      uploader = BatchUploader.new(wrapper_spec_config_path)
    end
  end

  describe 'Sending files' do
    it 'should run through each file in the YAML config and copy or move it to its destination(s)' do
      ...
      RestClient.should_receive(:post) do |url, params, settings|
        ...
        params['org_level2_id'].should eq(78)
      ...
      RestClient.should_receive(:post) do |url, params, settings|
        ...
        params.size.should eq(5)
        params['auth_token'].should eq('RfmknM43yYnZxtVPfAuH')
        params['org_level2_id'].should eq('invalid')
